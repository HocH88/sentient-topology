...
produces. We close with the following limitation: the rule-augmented
inhibition and seed channels are reported as augmentations rather than
labeled as ``learned,'' and human-subject validation of the topological
signatures remains future work.
\end{abstract}
...
\begin{enumerate}
    ...
    \item A context-conditional, type-biased, degree-normalized spreading
      activation that induces a stable active subgraph under sign-mixed
      edge weights (\S 3.2).
    ...
    \item A six-mechanism synaptic plasticity engine (Hebbian, Habituation,
      Sensitization, Homeostatic Scaling, Fading Affect Bias, Consolidation),
      each grounded in a specific neuroscience or affective-psychology
      reference, with hooks for additional invariant-preserving mechanisms
      described elsewhere and not claimed here (\S 3.4).
    \item A four-dimensional intrinsic-motivation vector
      $\mathbf{m} = (\mathrm{curiosity}, \mathrm{resolve}, \mathrm{express},
      \mathrm{deepen})$ derived from the topological asymmetry of the same
      activation map, with each dimension mapped to a specific neural
      circuit (\S 7), providing a measurable Level-2 layer above the
      Level-1 topological signature.
...
We instantiate the framework on an 8{,}000-node lemmatized SAN derived from
the 200-work corpus and report (i)~a static case study with baselines and
dimension-drop ablation, (ii)~three plasticity experiments reproducing the
Thompson--Spencer habituation curve, the Walker--Skowronski Fading Affect
Bias, and surprise-driven sensitization with effect signs consistent with
the source literature, (iii)~an \emph{intra-corpus affect-trajectory
differentiation} protocol with seven affect conditions (baseline plus six
of Panksepp's seven primary affects, LUST excluded) across six stimuli,
and (iv)~a $\pm 50\%$ parameter sensitivity analysis on the affect channels
(\S\ref{sec:exp}).

We also report two convergent \textbf{negative findings} on the relative
weight of the five topology dimensions. First, in the static
dimension-drop ablation across three seed concepts, Depth and Boundary
jointly carry $\sim$88\% of the context-discrimination signal; Density,
Symmetry, and Centrality together contribute $\sim$12\%
(\S\ref{subsec:baseline}). Second, in the intra-corpus affect-trajectory
experiment (\S\ref{subsec:affect_trajectory}), Boundary alone emerges as
the \emph{primary observable}, producing systematically consistent
directional differences for 13 of 21 affect pairs across at least four of
six stimuli; no other dimension exceeds six. The two findings converge:
the discriminative work of the framework is concentrated in the
persistence-based $B$ and geodesic $H$. We retain the five-dimensional
formalism for theoretical completeness.
...
contribute $\sim$12\%. We read this as a finding that strengthens, rather than undermines, the
TDA argument: the persistence-based $B$ and the geodesic $H$ -- which
classical graph-centrality summaries do not capture -- are the dimensions
that do the discriminative work.
...
next step.
(2)~Across both static ablation (\S\ref{subsec:baseline}) and affect-
trajectory differentiation (\S\ref{subsec:affect_trajectory}), the
discriminative weight of the five topology dimensions is asymmetric:
$D, S, C$ jointly contribute $\sim$12\% to context discrimination on the
static three-seed setup, and only Boundary attains cross-stimulus
directional consistency above six pairs (13 of 21) in the affect-trajectory
setup. We retain the five-dimensional formalism for theoretical completeness
but acknowledge that the framework's empirical power currently rests on
$H$ and especially $B$.
...
(5)~The plasticity experiments (\S 5.3) use small single-seed protocols
designed to verify mechanism signs; multi-seed replication is constrained
by the deterministic propagation noted in limitation~(11), so robustness
is reported instead via mechanism-by-mechanism sign agreement with the
source literature and is corroborated by the $\pm 50\%$ parameter
sensitivity analysis (\S\ref{subsec:sensitivity}).
